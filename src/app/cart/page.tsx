          ...
          {cart.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {cart.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row justify-between items-center border-b border-border-subtle py-6 gap-4 last:border-0"
                >
        ...
        <section className="pt-10 border-t border-border-subtle">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {recent.slice(0, 5).map((item) => (
              <div
                key={item.id}
                className="bg-card border border-border-subtle rounded-xl p-4 flex flex-col group transition-all hover:shadow-md"
              >
                <Link href={`/product/${item.id}`} className="flex-1">
                  <div className="aspect-square bg-white rounded-lg p-2 flex items-center justify-center mb-3 overflow-hidden border border-gray-50">
                    <img
                      src={item.thumbnail}
                      alt={item.title}
